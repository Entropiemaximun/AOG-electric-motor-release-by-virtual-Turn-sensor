...
#define SignalServo 5  // D5 comme  signal
#define EmbrayageServo 1
#define DebrayageServo 160
int AngleServo;

#define TurnAngle 1.5
...
uint8_t pulseCount1 = 0;  // Steering Wheel Encoder
...
void setup() {
  ...
  myServo.attach(SignalServo);  // attaches the servo on pin 5 to the servo object
...
void loop() {
  ...
  if (currentTime - lastTime >= LOOP_TIME) {
      ...
      if (steerConfig.ShaftEncoder) {
        ...
        if (!steerSwitch) {
          if (pulseCount) {
            ...
            if ((steerAngleOld + 25 < abs_AngleError) || gpsSpeed <= 1) {
              pulseCount = 0;
            } else if (abs_AngleError >= steerAngleOld && abs_AngleError > 3) {
      pulseCount += constrain(abs_AngleError, 0, 15      );
            ...
            if ((steerAngleOld + 25 < abs_AngleError) || gpsSpeed <= 2) {
              pulseCount1 = 0;
            } else if (abs_AngleError >= steerAngleOld && abs_AngleError > 3) {
              pulseCount1 += 5;
            } else {
            ...
            if (steerConfig.IsDanfoss) {
              //if (pulseCount >= steerConfig.PulseCountMax || pulseCount1 >= steerConfig.PulseCountMax) {
            ...
            if (gpsSpeed > 1) {
              if (sensorReading++ > 2000 / LOOP_TIME) {
    ...
    if (!steerSwitch  ) {
      Serial.print("steer limit ");
      Serial.print( steerConfig.PulseCountMax);
      Serial.print(" pulseCount ");
      Serial.print(pulseCount);
      Serial.print(" pulseCount1 ");
      Serial.print(pulseCount1);
      Serial.print(" abs_AngleErrorx10 ");
      Serial.print(abs_AngleError *10);
      Serial.print(" steerAngleOldx10 ");
      Serial.print(steerAngleOld * 10);
      Serial.print(" speed ");
    ...
    */
    ...
    if (AngleServo-- < EmbrayageServo) AngleServo = EmbrayageServo;
  ...
  */
  ...
  if (udpData[0] == 128 && udpData[1] == 129 && udpData[2] == 127)  //Data
  {
    ...
    else if (udpData[3] == 251)  //251 FB - SteerConfig
    {
      ...
      if (bitRead(sett, 3)) steerConfig.IsUseX_Axis = 1;
      else steerConfig.IsUseX_Axis = 0;
  ...
  if (udpData[0] == 0x80 && udpData[1] == 0x81 && udpData[2] == 0x7F)  //Data
  {
    ...
    else if (udpData[3] == 200)  // Hello from AgIO
    {
      ...
      if (bitRead(sett, 0)) aogConfig.isRelayActiveHigh = 1;
